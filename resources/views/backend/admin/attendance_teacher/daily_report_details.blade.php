...
<div class="row">
    @php
        $header_data =  "<h3> $app_settings->name</h3>" .
          "<h4>Teacher Daily Attendance Report</h4>" .
          "<h4>Date : $atten_date </h4>";

    @endphp
    <div class="col-md-6 offset-md-3 card" style="text-align: center; background: #fbfdfa">
        {!!  $header_data !!}
    </div>
</div>
<div class="row">
    <div class="col-md-12">
        <table id="summery" class="table table-bordered table-hover">
            <tbody>
            <tr>
                <td>Total Teacher : {{ count($data) }} </td>
                <td>Present Teacher : {{ $total_present }}</td>
                <td>Teacher in Leave: {{ $total_leave }}</td>
        ...
        table = $('#manage_all').DataTable({
            ...
            buttons: [
                ...
                },
                {
                    ...
                    titleAttr: 'PDF',
                    filename : 'Teacher_attendance_{{$atten_date}}',
                    exportOptions: {
                        columns: ':visible'
                    },

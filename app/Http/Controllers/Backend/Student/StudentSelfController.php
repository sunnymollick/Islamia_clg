...
class StudentSelfController extends Controller
{
    ...
    public function profile()
    {
        ...
        return view('backend.student.profile', compact('student', 'enroll'));
    }
...
//                    $student->email = $request->input('email');
    ...
    {
        $year = config('running_session');
        $enroll = Enroll::where('year', $year)->where('student_code', Auth::user()->std_code)->first();

        $exams = Exam::where('year', $year)->where('std_class_id', $enroll->section->stdClass->id)->orderBy('created_at', 'desc')->get();
...
//        dd($student_id);
        $year = config('running_session');
        $student = DB::table('enrolls')
            ->join('students', 'students.id', '=', 'enrolls.student_id')
            ->join('sections', 'sections.id', '=', 'enrolls.section_id')
            ->join('std_classes', 'std_classes.id', '=', 'enrolls.class_id')
            ->select('enrolls.*', 'students.std_code', 'students.name as student_name',  'students.year',
                'std_classes.name as class_name', 'sections.name as section_name')
            ->where('enrolls.student_id', $student_id)
            ->where('enrolls.year', $year)->first();
//        dd($students);
//        foreach ($students as $student) {
//            $class_id = $student->class_id;
//            $std_session = $student->year;
//            $section_id = $student->section_id;
...
//        }
        ...
        $data = array();
        $data['std_session'] = $student->year;
        $data['student_code'] = $student->std_code;
        $data['student_name'] = $student->student_name;
        $data['std_roll'] = $student->roll;
        $data['class_id'] = $student->class_id;
        $data['section_id'] = $student->section_id;
        $data['exam_id'] = $exam_id;
        $data['class_name'] = $student->class_name;
        $data['section_name'] = $student->section_name;
        $data['exam_name'] = $exam->name;
        $data['year'] = $year;
        $data['has_ct'] = $exam->ct_marks_percentage;
        $data['mmp'] = $exam->main_marks_percentage;
        $data['result'] = GenerateMarksheet::srExamMarksheet($exam_id, $student->class_id, $student->section_id, $student->std_code, $year);
//        dd($data['result']);

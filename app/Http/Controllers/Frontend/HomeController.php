...
use App\Models\AdmissionSscResult;
...
class HomeController extends Controller
{
   ...
   public function index()
   {
        ...
        $data = Page::whereIn('slug', [
            'our-history', 'chairman-message', 'principal-message'
        ])->get()->keyBy('slug');

        return View::make('frontend.index', compact('teacher', 'students', 'birthday', 'sliders', 'data', 'latest_news', 'teachers_notice'));
    }
    ...
    public function getSectionsManually(Request $request, $class)
    {
        if ($request->ajax()) {
            // dd($class);
            if ($class == 'HSC') {
                ...
                echo "<option  value='Business Studies'> Business Studies </option>";
                echo "<option  value='Humanities'> Humanities </option>";
            }elseif($class == 'HONOURS'){
                echo "<option value='' selected disabled> Select a section</option>";
                ...
                echo "<option  value='Islamic Studies'> Islamic Studies </option>";
            }
        } else {
            return response()->json(['status' => 'false', 'message' => "Access only ajax request"]);
        }
    ...
    public function onlineAdmissionStore(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($validator->fails()) {
                ...
                if ($request->admitted_class == 'HSC') {
                        ...
                        if ($rows == 0) {
                            ...
                            try {
                                ...
                                // $admission->college_roll =                          $request->input('college_roll');
                                ...
                                $admission->ssc_exam_roll = $request->input('ssc_exam_roll');
                                $admission->ssc_reg_no = $request->input('ssc_reg_no');
                                $admission->ssc_exam_board = $request->input('ssc_exam_board');
                                $admission->ssc_exam_session = $request->input('ssc_exam_session');
                                $admission->ssc_passed_year = $request->input('ssc_passed_year');
                ...
                }elseif ($request->admitted_class == 'HONOURS') {
                    ...
                    if ($rows == 0) {
                        ...
                        try {
                            ...
                            $admission->passed_school_name = $request->input('passed_school_name');
                            $admission->passed_college_name = $request->input('passed_college_name');
                            ...
                            $admission->ssc_exam_session = $request->input('ssc_exam_session');
                            $admission->ssc_passed_year = $request->input('ssc_passed_year');
                            $admission->grand_gpa = $request->input('ssc_gpa');

                            $admission->hsc_exam_roll = $request->input('hsc_exam_roll');
                            $admission->hsc_reg_no = $request->input('hsc_reg_no');
                            $admission->hsc_exam_board = $request->input('hsc_exam_board');
                            $admission->hsc_exam_session = $request->input('hsc_exam_session');
                            $admission->hsc_passed_year = $request->input('hsc_passed_year');
                            $admission->hsc_gpa = $request->input('hsc_gpa');
